...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  <url>
    ...
    <lastmod>${
      offer.validity?.end_date
        ? new Date(offer.validity.end_date).toISOString()
        : currentDate
    }</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.8</priority>
    <image:image>
      <image:loc>${escapeXml(
        `${baseUrl}/api/og?title=${encodeURIComponent(
          offer.title
        )}&bank=${encodeURIComponent(offer.bank)}`
      )}</image:loc>
      <image:caption>${escapeXml(
        `${offer.title} - ${offer.bank} Card Offer`
      )}</image:caption>
    </image:image>
  </url>`
    )
  ...
  ${banks
    .map(
      (bank) => `
  <url>
    <loc>${baseUrl}/bank/${encodeURIComponent(
  ...
  ${categories
    .map(
      (category) => `
  <url>
    <loc>${baseUrl}/category/${encodeURIComponent(
