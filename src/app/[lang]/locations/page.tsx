...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    locale: lang,
    path: '/locations',
    title: dict.pages.locationsTitle,
    description: dict.pages.locationsDescription,
    image: ogImageUrl({
      title: imageDict.pages.locationsTitle,
      subtitle: imageDict.tagline,
      locale: ogTextLocale(lang),
    }),
    imageAlt: dict.pages.locationsTitle,
  });
}
...
export default async function LocationsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const routes = townRoutes(active);
  ...
  const itemListJsonLd = {
    ...
    '@type': 'ItemList',
    name: dict.pages.locationsTitle,
    url: absoluteUrl(localizedPath(locale, '/locations')),
    ...
    <>
      ...
      <main id="main-content" className="min-h-screen bg-slate-50">
        <div className="container mx-auto max-w-5xl px-4 py-6 sm:py-10">
          ...
          <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">{dict.pages.locationsTitle}</h1>
          <p className="mt-2 max-w-2xl leading-relaxed text-gray-600">{dict.pages.locationsDescription}</p>

          <ul className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {routes.map((route) => {
              const offers = getOffersByTown(route.town, active);
              const categories = Array.from(new Set(offers.map((offer) => offer.category))).slice(0, 3);
              return (
                <li key={route.slug}>
                  <Link
                    href={localizedPath(locale, `/locations/${route.slug}`)}
                  ...
                  >
                    <span className="text-lg font-bold text-gray-900">{route.town}</span>
